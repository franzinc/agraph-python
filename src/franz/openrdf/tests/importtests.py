...
def test_add_doc_data_rules(conn, ex):
    ...
    statements = [
        stmt for stmt in get_statements(conn) if "load-meta" not in str(stmt[0])
    ]
...
def test_add_doc_data_prefix(conn, pref1, pref2):
    ...
    conn.addDocumentData({"a": "aa", "b": "bb"}, prefix={"a": pref1, "b": pref2})
    statements = get_statements(conn)
    assert len(statements) == 2
    assert statements == [
        [statements[0][0], p1.a, Literal("aa"), None],
...
def test_add_doc_data_prefix_in_keys(conn, pref1, pref2):
    ...
    conn.addDocumentData(
        {"a": "aa", "b": "bb"},
        keys={"a": DocumentKey(prefix=pref1), "b": DocumentKey(prefix=pref2)},
    )
    statements = get_statements(conn)
    assert len(statements) == 2
    assert statements == [
...
def test_add_doc_data_skip_key(conn):
    conn.addDocumentData({"a": "aa", "b": "bb"}, keys={"b": DocumentKey(skip=True)})
    assert len(get_statements(conn)) == 1
...
@pytest.mark.parametrize("excel_tab", ["excel-tab", csv.excel_tab])
def test_add_doc_data_csv_dialect(conn, excel_tab):
    conn.addDocumentData(
        'a\tb\tc\r\n"x""x"\t yy\tzz', DocFormat.CSV, csv_dialect=excel_tab
    )
    statements = get_statements(conn)
    assert 3 == len(statements)
    subject = statements[0][0]
...
def test_add_doc_data_csv_override_dialect(conn, doc, arg, value, dialect):
    conn.addDocumentData(doc, DocFormat.CSV, csv_dialect=dialect, **{arg: value})
    statements = get_statements(conn)
    assert 3 == len(statements)
    subject = statements[0][0]
    ns = conn.namespace(statements[0][1].getNamespace())
...
def test_add_doc_data_attributes(conn, ex, attr):
    conn.addDocumentData(
        {"p": "oo"}, base=ex(""), subject=ex.s, attributes={"test": "c"}
    )
...
def test_add_doc_encoding(conn, after):
    ...
    assert statements[0][2] == Literal("दुप🛁")
