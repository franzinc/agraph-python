...
import json
import time, math, operator, re, threading, urllib.request, urllib.parse, urllib.error
...
class Repository(Service):
    ...
    def addStatement(self, subj, pred, obj, context=None):
        ...
        nullRequest(self, "POST", "/statements", json.JSONEncoder().encode([[subj, pred, obj, context]]),
    ...
    def addStatements(self, quads, commitEvery=None):
        ...
        nullRequest(self, "POST", "/statements?" + urlenc(commit=commitEvery),
            json.JSONEncoder().encode(quads), contentType="application/json")
    ...
    def deleteStatements(self, quads):
        ...
        nullRequest(self, "POST", "/statements/delete", json.JSONEncoder().encode(quads), contentType="application/json")

    def deleteStatementsById(self, ids):
        nullRequest(self, "POST", "/statements/delete?ids=true", json.JSONEncoder().encode(ids), contentType="application/json")
    ...
    def registerEncodedIdPrefixes(self, registrations):
        ...
        nullRequest(self, "POST", "/encodedIds/prefixes",
            contentType="application/json", body=json.JSONEncoder().encode(body))
