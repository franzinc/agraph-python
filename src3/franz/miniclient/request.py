...
import pycurl
import json
import io, errno, urllib.request, urllib.parse, urllib.error, locale, re, os, time
...
def makeRequest(obj, method, url, body=None, accept="*/*", contentType=None, callback=None, errCallback=None, headers=None):
    ...
    if callback:
        ...
        def headerfunc(string):
            if status[0] is None:
                # we're getting bytes so we convert back to a string
    ...
    def process(self, string):
        ...
        def takeArrayAt(start):
            ...
            while True:
                ...
                try:
                    useArray(json.JSONDecoder().decode(string[start : end + 1].decode("utf-8")))
                    pos[0] = end + 1
                    return True
                except TypeError:
                    scanned = end + 1
