...
import pycurl
import os, urllib.request, urllib.parse, urllib.error, datetime, time, locale, io, subprocess, threading, warnings
...
def test_roundtrips():
    ...
    conn.addTriple('<http:object>', '<http:long>', 1234)
    ...
    def checkit(name, the_type, value):
        obj = conn.getStatements(None, '<http:%s>' % name, None).next().getObject().toPython()
