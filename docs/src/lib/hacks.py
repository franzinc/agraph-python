...
from sphinx.ext.autosummary import Autosummary, import_by_name, get_import_prefixes_from_env, get_documenter
from sphinx.ext.doctest import TestcodeDirective, TestsetupDirective, TestoutputDirective, SphinxDocTestRunner, DocTestBuilder
    ...
    class NewDirective(directive):
        ...
        def run(self):
            ...
            if self.options.get('if'):
                ...
                ns = dict((confval.name, confval.value)
                          for confval in app.config)
...
def setup(app):
    ...
    app.add_directive('condtestoutput',
                      conditionalize(CustomTestoutputDirective))
