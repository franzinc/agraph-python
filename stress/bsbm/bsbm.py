...
# User the administrative user to ensure there is an anonymous user
def ensure_anonymous(name):
    ...
    #conn.request('GET', '/users/anonymous/access', headers=headers)
    #assert 'read: true\n' in conn.getresponse().read()
...
def triples_to_product_count(triples):
    ...
    if triples <= 25000000:
	    return triples * 70812 / 250000000
...
def create_store(triples, name):
    ...
    #run_cmd('/usr/bin/env AG_PORT=%d /disk1/rjekel/agraph-4.2-client-python/stress/load/load -C -c "" -l %d -r %s bsbm-load-*.nt' % (AG_PORT, cpus(), name))
