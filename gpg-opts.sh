...
if [ -z "$PYPI_GPG_KEY" ]; then
    echo Usage: $0 gpg-key-name-or-id 1>&2
    exit 1
fi
...
if [ -n "${AG_GPG_PASSPHRASE}" ]; then
...
elif [ -n "${AG_GPG_PASSPHRASE_FILE}" ]; then
    ...
    if gpg --batch -o /dev/null --passphrase-fd 0 \
           --local-user "${PYPI_GPG_KEY}" -as - 2> /dev/null </dev/null; then
